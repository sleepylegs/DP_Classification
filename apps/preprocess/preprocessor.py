    ...
    def preprocess_trainset(self):
        ...
        *   none:
        ...
        try:
            ...
            data=self.drop_columns(data,['id'])
            # handle label encoding
            cat_df = self.feature_encoding(data)
            data = pd.concat([data, cat_df], axis=1)
            # drop categorical column
            #data = self.drop_columns(data, ['column name if needed'])
            ...
            self.X, self.y = self.split_features_label(data, label_name='diabetes_binary')
